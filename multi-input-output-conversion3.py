...
def real_fanout(collection, nodes):
    outputs = output_interface(collection, nodes)
    fan_out = 0
    for node in nodes:
...
def can_merge(a, b, nodes, max_fanin, max_fanout, max_different_outputs, x, y):
    ...
    if rfi > max_fanin:
        return False
    rfo = real_fanout(potential_match, nodes)
    ...
    if rsfo > max_different_outputs:
        return False
    # print(rsfo)
    # print max_different_outputs
    match_allowed = (
        (rfi <= max_fanin) and (rfo <= max_fanout) and (rsfo <= max_different_outputs)
    )
    print("Depth: A:" + str(x) + " B:" + str(y))
    return True
    same_topo_range = x == y
    #this would only be needed in case that the topo distance/ depth distance is greater than 1
    inputs_included = all_inputs_in(a, b, nodes) or all_inputs_in(b, a, nodes)
    if abs(x - y) <= 1:
        inputs_included = True
    if inputs_included or same_topo_range:
        return True
    return same_topo_range or inputs_included


def main():
    fanin = int(sys.argv[3])
    fanout = int(sys.argv[4])
    targetmerges = int(sys.argv[5])
    max_real_outputs = int(sys.argv[6])
    print(
        "Collecting "
        + str(sys.argv[1])
        + " to "
    ...
    process_file(
        sys.argv[1], sys.argv[2], fanin, fanout, targetmerges, max_real_outputs
    )

    ...
    if (line.rfind("INPUT", 0) == 0) {
      ...
      if (map.find(tokens[0]) == map.end()) {
        ...
        if (tokens[2] == "gnd") {
          ...
          vector<int> function_bits;
          function_bits.push_back(1);
          function_bits.push_back(1);
          assert(function_bits.size() == pow(2, curr_node->inputs.size()));
          curr_node->function_bits = function_bits;
        } else {
          if (tokens[2] != "LUT") {
            throw std::runtime_error("Unsupported gate type encountered. Only "
                                     "LUT, gnd, and vdd gates are supported");
          }
          ...
          vector<int> function_bits = convert_hex_to_binary(
              pow(2, curr_node->inputs.size()), tokens[3]);
          assert(function_bits.size() == pow(2, curr_node->inputs.size()));
          curr_node->function_bits = function_bits;
        }
      } else {
...
// Convertes the graph g into SHDL format and saves it as the given filename +
// "_SHDL"
...
void parse_bench(string str_filename) { parse_bench(str_filename, 1); }

/*
Converts a .bench file into a SHDL with fanout <= 2 that can be used in our UC
compiler. Additionally it is possible to create a SHDL file consisting of
multiple copies of the original .bench file. Inputs and outputs are adopted.
There is no interaction between the copies of the circuit.
*/
void parse_bench(string str_filename, int copies) {
  ...
  string outfilename = str_filename;
...
// Creates a graph with the given number of inputs, gates and outputs. Edges are
// randomly added only in ascending order of the vertices while respecting the
// maximum in- and outdegree. This is not a uniformly random chosen graph.
Graph *create_random_graph(unsigned int num_inputs, unsigned int num_gates,
  ...
  auto seed = duration_cast<milliseconds>(
                  high_resolution_clock::now().time_since_epoch())
                  .count();
  std::mt19937 gen(seed);
  std::uniform_int_distribution<size_t> rand_vertices =
      std::uniform_int_distribution<size_t>(num_inputs, num_vertices - 1);
  for (unsigned int i = 0; i < num_vertices - num_outputs; i++) {
    for (unsigned int j = 0; j < max_outdegree; j++) {
      ...
      if (target->inputs.size() <
          max_indegree) { // if it is higher, ignore this edge
...
int main(int argc, char *argv[]) {
  std::cout << "=== Converting Bench format to SHDL ===\n";
  parse_bench(argv[1]);
  std::cout << "=== Done converting Bench format to SHDL ===\n";
}
